...
#endif
#if BT_MUSIC_EFFECT_XDRC_EN && BT_MUSIC_EFFECT_DBG_EN
        eq_dbg_cb.xdrc_packet = xdrc_eq_name_compare((const char *)&eq_rx_buf[6]);
#endif
...
#if BT_TWS_EN && BT_MUSIC_EFFECT_XDRC_EN && BT_MUSIC_EFFECT_DBG_EN
        if ((bt_tws_is_connected() && eq_dbg_cb.rx_type && eq_dbg_cb.xdrc_packet) || (!eq_dbg_cb.rx_type && !bt_tws_is_slave())) {
            bt_tws_sync_eq_param();
            eq_tx_ack(band_num, 0, set_type);
            if (bt_tws_is_slave()) {
                return;
...
#endif
#if BT_MUSIC_EFFECT_XDRC_EN && BT_MUSIC_EFFECT_DBG_EN
    if (eq_dbg_cb.xdrc_packet) {
        bsp_xdrc_eq_param(eq_dbg_cb.xdrc_packet, band_cnt, eq_rx_buf);
    } else
#endif
    {
        if (set_type) {
//            #if MIC_DETECT_EN
//            if (MIC_IS_ONLINE()) {
                printf("local mic drc");
                mic_drc_set_by_param(band_cnt, (u32 *)&eq_rx_buf[10]);
//            }else
//            #endif
//            {

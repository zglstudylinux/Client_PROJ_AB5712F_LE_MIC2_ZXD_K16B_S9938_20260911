...
#if ADAPTER_AB_FOT_DEVICE_SUPPORT
static u8 fot_tws_save_data[FOT_TWS_DATA_LEN+20] AT(.fot_data.buf);
#endif
...
#if ADAPTER_AB_FOT_EN
    if(fot_var.con_type == FOTA_CON_BLE){
        packet_len = BLE_DEFAULT_MTU - 3;//ble_get_gatt_mtu();
    }else
#endif
    {
...
{
    #if WIRE_UPGRAD_EN
    if (is_wire_up_fot()) {     //有线升级的话，就return 0
        return 0;
    } else
    #elif ADAPTER_AB_FOT_DEVICE_SUPPORT
    {
        return 0;
    }
    #endif
...
{
    uint8_t *fot_tws_ptr = fot_tws_cmd;
    uint16_t fot_tws_ptr_pos = 0;
    uint16_t fot_tws_frame_len = 0;
    uint8_t continue_flag = 0;

    if(fot_tws_data_type == FOT_TWS_TYPE_DATA){
        memcpy(fot_tws_save_data, fot_tws_data, fot_tws_data_size);
        fot_tws_ptr = fot_tws_save_data;
    }

    while (fot_tws_data_size) {
        if (fot_tws_data_size > MAX_FRAME_LEN) {
            fot_tws_frame_len = MAX_FRAME_LEN;
        } else {
            fot_tws_frame_len = fot_tws_data_size;
        }
        if (continue_flag) {
            for (uint16_t i = fot_tws_data_size; i > 0; i--) {
                fot_tws_ptr[fot_tws_ptr_pos+i] = fot_tws_ptr[fot_tws_ptr_pos+i-1];
            }
            fot_tws_ptr[fot_tws_ptr_pos] = FOT_OUT_DATA_CONTINUE;
            gap_user_channel_send_buf(0, fot_tws_ptr+fot_tws_ptr_pos, fot_tws_frame_len+1);
            gap_user_channel_send_buf(1, fot_tws_ptr+fot_tws_ptr_pos, fot_tws_frame_len+1);
        } else {
            gap_user_channel_send_buf(0, fot_tws_ptr+fot_tws_ptr_pos, fot_tws_frame_len);
            gap_user_channel_send_buf(1, fot_tws_ptr+fot_tws_ptr_pos, fot_tws_frame_len);
        }

        fot_tws_data_size -= fot_tws_frame_len;
        fot_tws_ptr_pos += fot_tws_frame_len;
        if (continue_flag) {
            fot_tws_ptr_pos += 1;
        }
        continue_flag = 1;
...
AT(.text.fot.update)
static void bt_fot_tws_resp(void)
{
    u8 data[3];

    data[0] = FOT_TWS_RESP_TAG;

    tws_fot_fill_in_data(data, 1);
...
#if ADAPTER_AB_FOT_EN
    if(fot_var.con_type == FOTA_CON_BLE){
        ble_update_conn_param(0, 12, 0, 400);
    }
#endif
...
AT(.text.fot.update)
void fot_tws_cmd_deal(u8 index, u8 *buf, u16 len)
{
    ...
    switch(cmd){
        ...
        case FOT_TWS_FILE_ADDR:
            fot_var.tws_addr_index |= BIT(index);
            memcpy(&remote_addr, &buf[2], 4);
            if (fot_var.tws_addr_index == CONNECTED_INDEX) {
                fot_var.tws_addr_index = 0;
...
AT(.text.fot.cache)
void fot_tws_recv_proc(u8 index, u8 *buf, u16 len)
{
    u32 addr;
    u32 recv_data_len;
   ...
   if(buf[FOT_CMD_POS] == FOT_OUT_DATA_START){
        ...
        if(fot_var.remain_len){
            ...
            memcpy(&fot_data[fot_var.data_pos], &buf[1], recv_data_len);
            fot_var.data_pos += recv_data_len;
        }
        ...
        fot_tws_cmd_deal(index,buf,len);
    }else if(buf[FOT_CMD_POS] == FOT_TWS_RESP_TAG){
        fot_var.tws_resp_index |= BIT(index);
        if (fot_var.tws_resp_index == CONNECTED_INDEX) {
            fot_var.tws_resp_index = 0;
            tws_fot_resp();
        }
    }
...
{
    fot_tws_recv_proc(index, data_ptr, size);
    return 0;
}
...
void fot_tws_update_data_write(u8 *data, u32 addr, u32 len)
{
    ...
    fot_tws_data[FOT_CMD_POS] = cmd;
    fot_tws_data[FOT_SEQ_POS] = seq;
    memcpy(&fot_tws_data[FOT_ADDR_POS], &addr, 4);
    memcpy(&fot_tws_data[FOT_DATA_LEN_POS], &len, 4);
    memcpy(&fot_tws_data[DATA_START_POS], data, len);

    fot_tws_data_size = len + DATA_START_POS;
    fot_tws_data_type = FOT_TWS_TYPE_DATA;
    bt_tws_sync_fot_data();
}

//用于usb输入升级的时候,给从机传输file hash
AT(.text.fot.update)
void fot_tws_update_file_hash_set(u32 file_hash)
{
    fot_var.remote_ver = 0xFFFF;
    fot_var.hash       = file_hash;
    fot_var.addr       = 0;
    fot_tws_sync_update_info();
}
...
{
    return fot_var.tws_rsp;
}

AT(.text.fot.update)
uint8_t fot_tws_remote_err_get()
{
    return fot_var.tws_remote_err;
}

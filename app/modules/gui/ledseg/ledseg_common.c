...
void disp_bat(void)
{
    ...
    {
        {
            //printf("sys_cb.bat_level = %d \n",sys_cb.bat_level);
            if((sys_cb.bat_level <= 1)||(sys_cb.lowbat_flag==1))      //半格闪烁
            {
                ledseg_buf[4] |= ICON_VBAT;
                if(box_cb.flicker.sta)
                {
                	ledseg_buf[4] |= ICON_VBAT3;
                }
            }
            else if(sys_cb.bat_level <= 2) //半格常亮
            {
                ledseg_buf[4] |= ICON_VBAT;
                	ledseg_buf[4] |= ICON_VBAT3;

            }
			else if(sys_cb.bat_level <= 3) //半格常亮
            {
                ledseg_buf[4] |= ICON_VBAT;
                	ledseg_buf[4] |= ICON_VBAT3;
                	ledseg_buf[4] |= ICON_VBAT2;
            }
            else //if(sys_cb.bat_level > 3) //满格常亮
            {
                ledseg_buf[4] |= ICON_VBAT;
                ledseg_buf[4] |= ICON_VBAT1;
                ledseg_buf[4] |= ICON_VBAT2;
                ledseg_buf[4] |= ICON_VBAT3;
            }
        }

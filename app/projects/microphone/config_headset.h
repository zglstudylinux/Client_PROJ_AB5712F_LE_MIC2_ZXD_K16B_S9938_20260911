...
#ifndef USER_CONFIG_DESKTOP_MIC_H
#define USER_CONFIG_DESKTOP_MIC_H
...
#define USER_C10_CONFIG                 1                       //C10专用配置
#define USER_CF_UI_CONFIG               0                       //用户专用UI配置
...
#define WIRELESS_CON_VERS                       4               //传输机制版本（0=V1, 1=V2, 2=V3)
...
#define WIRELESS_SPK_TX_EN                      1               //适配器是否支持SPK发射（下行音频）
...
#define WIRELESS_MIC_ENC_MAX_US                 1300            //编码运算时长（单位us,主频160M:850,主频120M:1250,主频80M: 1300）
...
//无线SPK配置（下行音频，ADAPTER --> DEVICE）
#define WIRELESS_SPK_SAMPLE_RATE_SELECT         SAMPLE_RATE_48K //无线spk采样率选择，暂时支持16k,24k,32k,48k
#define WIRELESS_SPK_SAMPLES_SELECT             120             //无线spk每帧采样样点数选择
#define WIRELESS_SPK_CHANNEL_SELECT             2               //无线spk声道选择，1为单声道，2为双声道
#define WIRELESS_SPK_FRAME_SIZE                 50              //每帧压缩后大小（FRAME_SIZE*LINK_NB*RETRY_NB暂时不能超过250byte）
#define WIRELESS_SPK_DUMP_PER_BER               0
#define WIRELESS_SPK_LOCAL_OUTPUT_EN            0               //是否支持本地dac输出SPK音频（调试SPK通路用）
#define WIRELESS_SPK_ENC_MAX_US                 1600            //编码运算时长(提前一点, 预留时间给MIC解码)
#define WIRELESS_SPK_DEC_MAX_US                 1600            //解码运算时长
#define WIRELESS_SPK_RETRY_NB                   4               //传输次数
...
#define WIRELESS_MIC_AINS4_EN                   1               //AINS4算法使能  (发射端处理，默认开启后主频会抬至160M)
...
#define WIRELESS_MIC_DAC_OUTPUT_EN              0               //发射端是否输出MIC声音
#define WIRELESS_SPK_DAC_OUTPUT_EN              1               //发射端是否输出SPK声音
...
#define ADAPTER_DAC_OUTPUT_EN                   1               //适配器dac输出(默认仅输出无线MIC声音)
#define ADAPTER_SPK_OUTPUT_EN                   0               //适配器是否输出SPK音频(用于监听USB\I2S\AUX, 需要同时打开ADAPTER_DAC_OUTPUT_EN)
...
#define ADAPTER_USB_MIC_RX_EN                   1               //接收端是否打开usb mic#if FUNC_TEST_PRINT//调试打印
#define ADAPTER_USB_SPK_TX_EN                   0               //适配器是否支持USB 下行音频
#define ADAPTER_AUX_TX_EN                       1               //适配器是否支持AUX 下行音频
#define ADAPTER_I2S_TX_EN                       0               //适配器是否支持I2S 下行音频(ADAPTER_USB_SPK_TX_EN、ADAPTER_AUX_TX_EN、ADAPTER_I2S_TX_EN, 三选一)
#define ADAPTER_AUDIO_FADE_IN_EN                1               //适配器下行音频是否使用淡入功能(避免有音频输入时, 连接上瞬间, 声音比较突兀)
...
#define ADAPTER_USB_MIC_STEREO_EN               0               //接收端usb mic是否支持真立体声
...
#define EQ_APP_EN                       1            //是否支持APP设置EQ参数(USB HID方式)
#define EQ_APP_NUM                      8            //EQ点数
...
#define BREATH_LED_EN                   0           //发射端呼吸灯使能(默认PB4)
...
#define I2C_SCL_IN()                    GPIOADIR |= BIT(6);
#define I2C_SCL_OUT()                   {GPIOADIR &= ~BIT(6); GPIOADE |= BIT(6);}
#define I2C_SCL_H()                     GPIOASET = BIT(6);
#define I2C_SCL_L()                     GPIOACLR = BIT(6);
#define I2C_SDA_IN()                    {GPIOADIR |= BIT(7); GPIOAPU |= BIT(7);}
#define I2C_SDA_OUT()                   {GPIOADIR &= ~BIT(7); GPIOADE |= BIT(7);}
#define I2C_SDA_H()                     GPIOASET = BIT(7);
#define I2C_SDA_L()                     GPIOACLR = BIT(7);
#define I2C_SDA_IS_H()                  (GPIOA & BIT(7))
...
#define I2S_AUDIO_IN_EN                 (WIRELESS_MIC_I2S_INPUT_EN ||ADAPTER_I2S_TX_EN)    //是否使能I2S输入音频
...
#define SPI_BAUD_RATE                   4000000        //SPI波特率2M

...
void ledseg_disp_welcome(void)
{
    ...
    ledseg_buf[2] = T_LEDSEG_N;
}

AT(.text.display.ledseg)
void ledseg_disp_volume(void)
{
    ledseg_disp_number(sys_cb.soft_gain_level);
    ledseg_buf[0] = T_LEDSEG_U;
    ledseg_buf[1] = T_LEDSEG__;

    if(sys_cb.wireless_id_sta)
    {
        ledseg_buf[4] |= ICON_WIR_B;
    }
    else
    {
        ledseg_buf[4] |= ICON_WIR_A;
    }
...
AT(.text.display.ledseg)
void ledseg_disp_echo(void)
{
    ledseg_disp_number(echo_delay_level_get());
    ledseg_buf[0] = T_LEDSEG_E;
    ledseg_buf[1] = T_LEDSEG__;
...
AT(.text.display.ledseg)
void ledseg_disp_device()
{
    ledseg_buf[0] = T_LEDSEG_2;
    ledseg_buf[1] = T_LEDSEG_4;
    if(sys_cb.wireless_id_sta)
    {
        ledseg_buf[2] = T_LEDSEG_0;
        ledseg_buf[3] = T_LEDSEG_8;
        ledseg_buf[4] |= ICON_WIR_B;
    }
    else
    {
        ledseg_buf[2] = T_LEDSEG_0;
        ledseg_buf[3] = T_LEDSEG_2;
        ledseg_buf[4] |= ICON_WIR_A;
    }

    ledseg_buf[4] |= (ICON_BT|ICON_MHZ);
}

...
`0x85` 的 PCLKSEL=010 → **TMR1 跑 24M 晶振（固定频率；PCLKSEL 源名 xosc26m 系历史命名，板载晶振实为 24M），与系统主频无关**。
...
| 连接后 | 196.608M | 0x85（消隐开启） | 0x172~0x384（370~900） | 15~39µs / 1000µs ≈ **1.6~3.9%** | lib 消隐 → 暗 |

- 中断延迟：待机 max=34µs、连接后 max=4µs → **RF 没有挤占扫描中断**，"中断挤占"候选排除；
- 消隐窗口跑 24M 晶振固定时钟 → 与主频无关，"主频推暗"候选排除；
- **真因：连接后 `ledseg_ajust()` 按原厂设计启动了 TMR1 消隐（占空压到 1.6~3.9%），且 PR 随显示内容在 370~900 间跳变（约 2.4 倍亮度波动），叠加射频抖动 → 视觉上的"变暗 + 持续闪烁"；待机档实测未武装消隐（全亮）是异常基准，与源码行为不符，机制在闭源部分无法进一步确认。**
...
2. 原厂消隐路径诊断固件实测（每 5 秒统计 5000 次扫描）：
   ...
   - **连接后（主频抬至 196.608MHz，即"160M"档实际频率）**：稳定窗口内每次扫描均为 `TMR1CON=0x85`、`TMR1PR=370~900`、`tmr1isr=1`，消隐工作，占空骤降为 1.6~3.9%。
3. 用户感知的"变暗+闪烁"= 待机全亮（消隐未生效）与连接后原厂设计亮度（消隐生效）之间的落差，叠加 PR 随显示内容在 370~900 间跳变（约 2.4 倍亮度波动）与射频抖动。
4. 同时排除的候选：RF 中断挤占扫描（tick 中断进入延迟实测 max 4~54µs，连接后反而更小）；消隐窗口随主频缩短（TMR1 时钟源为固定 24M 晶振）；显示内容缺失（`ledseg_cb.buf` 七槽镜像恒为 "2402" 内容）。
...
> 排查过程中被证据修正过的推测：①"TMR1 跑系统主频域、抬频后窗口缩短"（手册证实时钟源为固定 24M 晶振（xosc26m 历史命名））；②"lib 按主频档位门控消隐"（原厂源码无门控）；③"lib 版本与源码不一致"（反汇编中的两次"调用"实为 `-msave-restore` 的序言/尾声辅助函数）。
...
flowchart TD
    ...
    G --> H[查手册 Register 4-1<br/>PCLKSEL=010 → xosc26m_clk 固定24M]
    H --> I[❌ 假设1推翻: 消隐窗口与主频无关]
    I --> J[只读诊断固件<br/>TMR0CNT测中断延迟 + ISR采样TMR1CON/PR]
    J --> K{数据判读}
    K --> L[tick_lat max=4~34us<br/>❌ 候选2排除]
    K --> M[待机 CON=0 全亮占空100%<br/>连接后 CON=0x85 PR=370~900 占空1.6~3.9%]
...
sequenceDiagram
    ...
    alt 待机档 24M
        ...
        AJ->>SET: 点亮本 COM
        T1-->>ISR1: 19~35us 后溢出
        ISR1->>SET: ledseg_7p7s_clr() 关屏 → 槽内占空仅 1.6~3.9%（暗）
    end
